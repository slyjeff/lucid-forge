...
**Skills install banner:** On launch, the app checks whether all LucidForge Claude Code skills are present in either `~/.claude/commands/` (legacy flat `.md` files) or `~/.claude/skills/<name>/SKILL.md` (current layout). If skills are not found in either location, a dismissible banner appears below the header row with an **Install** button. Clicking Install copies the bundled skill files to `~/.claude/commands/` and hides the banner. The banner reappears on the next launch if skills are still missing. Dismissed without installing = reappears next session.
